/**
 * AI 生成宠物提示词（客户端本地版，自平台后端 prompt-agent.ts 移植）：
 *   - runPromptAgent：把用户一句话描述加工为图像生成模型可直接使用的绘画提示词
 *     （智谱 glm-4-flash 免费 → 本地模板兜底，不阻断主流程；Key 从 config.aiGen 传入）
 *   - 五状态视频动作提示词（视频模型只负责动效；背景/外观约束固定钉死）
 */

/** 可选画风（与平台同款列表；图像模型对质感词敏感） */
...
/** 字段钳制：保证智能体产出安全可用。stylePrompt 追加为确定性后缀 */
...
/** 调用 OpenAI 兼容 /chat/completions 输出 JSON（智谱协议） */
...
/**
 * 运行提示词智能体：智谱 glm-4-flash（免费，用户自备 Key）→ 本地兜底。
 * @param description 用户原始描述
 * @param opts.stylePrompt 画风提示词（PAINT_STYLES，可空）
 * @param opts.zhipuKey 智谱 Key（config.aiGen.zhipuKey，可空——为空直接走本地模板）
 */
export async function runPromptAgent(description: string, opts: { stylePrompt?: string; zhipuKey?: string } = {}): Promise<PromptAgentResult> {
  const userPrompt = [
    opts.stylePrompt ? `画风倾向：${opts.stylePrompt}` : '',
    `用户描述：${description}`,
  ].filter(Boolean).join('\n');

  const zhipuKey = opts.zhipuKey?.trim();
  if (zhipuKey) {
    try {
      return clampResult(
        await callProvider('https://open.bigmodel.cn/api/paas/v4', zhipuKey, 'glm-4-flash', userPrompt),
        description,
        opts.stylePrompt,
      );
    } catch {
      // Key 失效/限流/输出异常：走本地模板兜底
    }
  }
...
/** 五状态视频动作提示词（视频模型只负责动效；背景/外观约束固定钉死） */
export const STATE_VIDEO_PROMPTS: Record<string, string> = {
...
export function withVideoSuffix(action: string): string {
