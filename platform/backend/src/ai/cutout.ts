/**
 * 绿幕抠图管线（供基准图与视频截帧共用）。
 * 职责：接收纯色浅绿底（#E9FFEB）原始立绘/视频帧，在服务端去背，返回只有主体的透明 PNG。
 * 算法（针对「浅色主体 + 浅绿背景」防误删设计，经三轮真实图像实测修复）：
 *   ① 洪泛：从四边清除与背景色差在容差内的 4 邻接连通像素（逐行左右边缘基准，容忍幕布渐变）；
 *   ② 腐蚀 + 边缘连通过滤（核心）：浅色主体与背景色差极小处，洪泛会沿抗锯齿细缝「泄漏」进
 ...
 *  图像模型常无视平涂要求，在浅绿底上画出绿色光晕/脚下椭圆阴影，其色差远超洪泛
