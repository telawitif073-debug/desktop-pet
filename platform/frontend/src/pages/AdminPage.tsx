...
import { Alert, Button, Card, Empty, Image, Pagination, Space, Spin, Tabs, Tag, Typography, message } from 'antd';
...
export default function AdminPage({ user, preview = false }: { user: User | null; preview?: boolean }) {
  ...
  useEffect(load, [type, page, preview]);

  if (user?.role !== 'admin' && !preview) return <Navigate to="/" replace />;
